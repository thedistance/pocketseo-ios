...
class APIManager {
    ...
    func mozscapeLinksForString(requestURLString:String, requestURLParameters: LinkSearchConfiguration, page:UInt, count:UInt = 25) -> SignalProducer<[MZMozscapeLinks], NSError> {
        ...
        //let parameters = requestURLParameters.mozscapeRequestParameters
        let parameters = LinkSearchConfiguration.distanceLinkSortyByDA().mozscapeRequestParameters
        
        let combinedParameters = parameters + ["SourceCols":String(colsValue), "LinkCols": String(MZLinksKey.AnchorText.colValue)]
